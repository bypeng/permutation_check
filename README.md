...
這是一個為 PTT TAR 13（實境解謎遊戲）的最終回憶任務：《屬於參賽者的賴和詩牆》設計的即時驗收與戰況轉播系統。它基於 Google Apps Script (GAS) 建立，並整合了 Google Sheets 作為資料庫以及 Line Bot 作為即時通知工具。這個系統的實作過程搭配了 Gemini 進行協作。
...
2.  **生成資料**：執行這兩個 Python 腳本，產生 `poem_correct.json` 、 `poem_confuse.json` 和 `player.json` 。
3.  **自動建構**：執行 `build_gs_file.py` 腳本。此腳本會讀取 `poem_correct.json` 、 `poem_confuse.json` 和 `player.json`，然後將其內容填入 `Code.gs.template` 中，最終生成一份可以直接使用的 `Code.gs` 檔案。
